...
describe('Background Core', () => {
  ...
  describe('handleCommand', () => {
    it('should call the correct handler for a known command type', async () => {
      ...
      // Reset browserTabId for this test
  ...
  describe('setupMessageHandler', () => {
    ...
    it('should handle EXECUTE_COMMAND messages', async () => {
      ...
      // Mock handleCommand directly
      const originalHandleCommand = handleCommand;
      global.handleCommand = jest.fn().mockResolvedValue({ success: true });
      ...
      global.handleCommand = originalHandleCommand;
    });
    ...
    it('should handle errors in EXECUTE_COMMAND messages', async () => {
      ...
      // Mock handleCommand to throw an error
      const originalHandleCommand = handleCommand;
      const error = new Error('Test error');
      global.handleCommand = jest.fn().mockRejectedValue(error);
  ...
  describe('initialize', () => {
    it('should set up message handler and extension handler', () => {
      ...
      const originalSetupMessageHandler = setupMessageHandler;
      const originalSetupExtensionHandler = setupExtensionHandler;
      
      // Replace with mocks
      global.setupMessageHandler = jest.fn();
      global.setupExtensionHandler = jest.fn();
      
      // Call initialize
      initialize();
      
      // Check if our mocks were called
      expect(global.setupMessageHandler).toHaveBeenCalled();
      expect(global.setupExtensionHandler).toHaveBeenCalled();
      expect(utils.chromeAPI.storage.local.get).toHaveBeenCalled();
      
      // Restore original functions
      global.setupMessageHandler = originalSetupMessageHandler;
      global.setupExtensionHandler = originalSetupExtensionHandler;
    });
