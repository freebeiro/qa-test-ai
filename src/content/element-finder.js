...
export function findNearestInput(element, targetText = '') {
  // Look for inputs that are siblings or children
  const parent = element.parentElement;
  if (parent) {
    // First try to find an email input if the target text is 'email'
    if (targetText.toLowerCase() === 'email') {
      const emailInput = Array.from(parent.querySelectorAll('input[type="email"]'))
        .find(input => isElementVisible(input));
      if (emailInput) return emailInput;
    }
    
    // Then try other input types
    const inputs = Array.from(parent.querySelectorAll('input, textarea'));
    const siblingInput = inputs.find(input => isElementVisible(input));
...
export function findInputByText(targetText) {
  ...
  if (labelMatch && labelMatch.htmlFor) {
    ...
    if (nearText) {
      ...
      input = findNearestInput(nearText, targetText);
    }
  }
  ...
  if (!input) {
    // First try to find an email input if the target text is 'email'
    if (targetText.toLowerCase() === 'email') {
      input = Array.from(document.querySelectorAll('input[type="email"]'))
        .find(input => {
          const placeholder = input.getAttribute('placeholder') || '';
          const name = input.getAttribute('name') || '';
          const ariaLabel = input.getAttribute('aria-label') || '';
          return isElementVisible(input) && (
            placeholder.toLowerCase().includes(targetText.toLowerCase()) ||
    ...
    // If no email input found, try other input types
    if (!input) {
